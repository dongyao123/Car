...
var Course1 = require("../models/leixing.js");
    ...
    form.parse(req, function(err, fields) {
        Course.add(fields,function(err,r){
            if(err){
                ...
                console.log(r,"AAAAAA")
                res.json({"r":r})
            }
        })
    });
}
exports.addleixing=function (req, res) {
    var form = new formidable.IncomingForm();
    form.parse(req, function(err, fields) {
        Course1.add(fields,function(err,r){
            if(err){
                res.send("提交失败！");
            }else{
                res.json({"r":r})
...
exports.deleteleixing=function (req, res) {
    var form = new formidable.IncomingForm();
    form.parse(req, function(err, fields) {
        var needToDelete = JSON.parse(fields.needToDelete);
        Course1.delete(needToDelete,function(err,n){
            if(err){
                res.send("-1");
            }else{
                res.send(n.toString());
...
exports.allcl=function (req, res) {
    Course1.getAll(function(results){
        res.json({"results" : results});
    })
}
exports.xinchelei=function (req, res) {
    Student.getAllche(function(results){
        res.json({"results" : results});
    })
}

exports.allcl2=function (req, res) {
    Course1.getAll(function(results){
        res.json({"results" : results});
    })
}
exports.allleixing=function (req, res) {
    Course1.getAll(function(results){
...
exports.changeleixing=function (req, res) {
    var form = new formidable.IncomingForm();
    form.parse(req, function(err, fields) {
        Course1.change(fields,function(err,r){
            res.send("");
        });
    });
}
...
exports.zuche=function (req, res) {
    ...
    form.parse(req, function(err, fields, files) {
        ...
        lishi.add(json,function (err, r,rr) {
            // console.log(rr,"A")
            // res.json({"rr":rr})
        })
